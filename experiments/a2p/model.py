...
class Audio2Pose(pl.LightningModule):
    def __init__(
        ...
        self.fps = int(fps)
        self.heat_hw = tuple(heat_hw)  # harmless to keep around
    ...
    def _cal_and_log_loss(self, batch, pred, label: str):
        ...
        # 3) Velocity loss
        if pred.size(1) > 1:
            dv_pred = (pred[:, 1:] - pred[:, :-1]) * self.fps  # [B,T-1,2,21,2]
            dv_gt = (gt[:, 1:] - gt[:, :-1]).detach() * self.fps
        ...
        loss = weight_pos * l_pos

        self.log_dict(
            # {f"{label}/l_pos": l_pos, f"{label}/l_bone": l_bone, f"{label}/l_vel": l_vel, f"{label}/reg_loss": loss},
            {f"{label}/l_pos": l_pos, f"{label}/reg_loss": loss},
    ...
    def _disc_inputs(self, pred_xy, gt_xy):
        ...
        """
        f_pose = _flatten_tp22(pred_xy)  # [B,T,84]
        r_pose = _flatten_tp22(gt_xy)  # [B,T,84]
        f_mot = _flatten_tp22(_to_motion_delta(pred_xy)) * self.fps  # [B,T-1,84]
        r_mot = _flatten_tp22(_to_motion_delta(gt_xy)) * self.fps  # [B,T-1,84]
    ...
    def validation_step(self, batch, _):
        ...
        with torch.no_grad():
            ...
            if self.use_gan:
                gt = batch["target"].float().to(self.device)
                g_adv, d_loss = self._gan_losses(pred, gt)  # D is in eval mode; no grads
                total = reg_loss + self.curr_lambda_gan * g_adv

                # log val metrics & a total that matches training's objective
                self.log_dict(
                    {"val/g_loss_adv": g_adv, "val/d_loss": d_loss, "val/loss": total},
                    prog_bar=True, on_epoch=True, on_step=False
                )
                return total
            else:
                # no GAN: total == reg
                self.log("val/loss", reg_loss, prog_bar=True, on_epoch=True, on_step=False)
                return reg_loss
